...
def run_portfolio_engine(
    ...
    *,
    ...
    for payload in symbol_payloads:
        ...
        # Add calendar/diagonal lifecycle roll suggestions (already generated by orchestrator)
        lifecycle_signals = result.get("positions", {}).get("calendar_diagonal_lifecycle", [])
        cal_roll_suggestions = [s["roll_suggestion"] for s in lifecycle_signals
                                if s.get("roll_suggestion") and s.get("action") != "HOLD"]
        all_lifecycle_rolls  = rolls + cal_roll_suggestions
        result["roll_suggestions"] = all_lifecycle_rolls
        all_roll_inputs.extend(all_lifecycle_rolls)
