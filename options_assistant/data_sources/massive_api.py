...
def get_atr_14(
    ...
    """
    Fetch daily OHLC candles from Polygon /v2/aggs and compute ATR-14.

    Returns (atr_14, atr_prior) where:
      atr_14   — average true range over the most recent 14 trading days
      atr_prior — average true range over the 14 days before that (days 15-28)

    Falls back to (3.0, 3.0) if the endpoint is unavailable or returns no data.

    This endpoint is available on all Polygon tiers including the free tier.
    """
    from datetime import date, timedelta

    end_date   = date.today().isoformat()
    start_date = (date.today() - timedelta(days=lookback_days + 10)).isoformat()

    try:
        data = _get(
            f"/v2/aggs/ticker/{symbol.upper()}/range/1/day/{start_date}/{end_date}",
            params={"adjusted": "true", "sort": "asc", "limit": lookback_days + 10},
            session=session,
        )
    except Exception:
        return 3.0, 3.0
    ...
    for i in range(1, len(results)):
        h = float(results[i].get("h", 0) or 0)
        l = float(results[i].get("l", 0) or 0)
        pc = float(results[i - 1].get("c", 0) or 0)
    ...
    prior  = trs[-28:-14] if len(trs) >= 28 else (trs[:-14] if len(trs) > 14 else trs)

    atr_14    = sum(recent) / len(recent)
    atr_prior = sum(prior) / len(prior) if prior else atr_14

    return round(atr_14, 4), round(atr_prior, 4)
